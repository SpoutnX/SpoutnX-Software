...
try:
    ...
    print("User folder---Failed")
    print("User folder---Create=True")

except:
    print("User folder---ok")


testConfig = os.path.exists("/home/" + user + "/.SpoutnX/config.ini")
if testConfig is False:
    ...
    with open(path5, 'w') as f:
        config.write(f)
    print("Config file---Create=True")
if testConfig is True:
...
if test is True:
    ...
    config = configparser.ConfigParser()
    config.read(path5)
    varLang = config.get("User Data", "Langage")
    if varLang == "Francais":
        shutil.copy(
            '/usr/lib/spoutnx/user_langage_fr.sptx', sourcelang)
    if varLang == "English":
        shutil.copy(
            '/usr/lib/spoutnx/user_langage_en.sptx', sourcelang)
    if varLang == "Espanol":
        shutil.copy(
            '/usr/lib/spoutnx/user_langage_esp.sptx', sourcelang)
    if varLang == "Portugues":
        shutil.copy(
            '/usr/lib/spoutnx/user_langage_po.sptx', sourcelang)
if test is False:
    if test2 is False:
        print("sourcelang----Failed")
        os.mkdir("/home/" + user + "/.SpoutnX/lang")
    shutil.copy(
        '/usr/lib/spoutnx/user_langage_fr.sptx', sourcelang)
    print("sourcelang----create=True")
...
listeObjectifs.move(50, 160)
listeObjectifs.setFixedSize(QSize(400, 160))
...
def help1():
    webbrowser.open_new(
        r"https://spoutnx.wixsite.com/spoutnx/documentation")
    ...
    if objectif2 == L[75]:
        ...
        def text_changed3(tes2):
            ...
            if tes == "Français":
                ...
                parser.set('User Data', 'Langage', "'Francais'")
            if tes == "English":
                shutil.copy(
                    '/usr/lib/spoutnx/user_langage_ang.sptx', path4)
                parser.set('User Data', 'Langage', "'English'")
            if tes == "Espanol":
                shutil.copy(
                    '/usr/lib/spoutnx/user_langage_esp.sptx', path4)
                parser.set('User Data', 'Langage', "'Espanol'")
            if tes == "Portugues":
                shutil.copy(
                    '/usr/lib/spoutnx/user_langage_por.sptx', path4)
                parser.set('User Data', 'Langage', "'Portugues'")
            parser.write(file)
            file.close()
...
app.lastWindowClosed.connect(myExitHandler)
