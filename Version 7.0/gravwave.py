...
def swarsResults4():
    ...
    if S[0] == 4 and S[1] == 1:
        ...
        def Model_OG_plot():
            import plotGravwaveCbA
            plotGravwaveCbA.printModel()

        btnModelisation = QPushButton(L[217])
        btnModelisation.setParent(winResults)
        btnModelisation.move(200, 450)
        btnModelisation.setFixedSize(QSize(150, 30))
        btnModelisation.setFont(font2)
        btnModelisation.pressed.connect(Model_OG_plot)
        btnModelisation.show()
...
#
# _____________________CARACTERISTIQUES__________________________________
...
def waves():
    ...
    def value_changed_DistObs(exp_Dist2):
        global exp_Dist
        exp_Dist = exp_Dist2
    ...
    labR = QLabel(L[160])      # Distance avec l'observateur
    ...
    spinbox_Dist = QDoubleSpinBox()
    spinbox_Dist.setRange(0, 1000)
    spinbox_Dist.setParent(winCaracteristiques)
    spinbox_Dist.setFixedSize(QSize(80, 20))
    spinbox_Dist.move(290, 360)
    spinbox_Dist.setPrefix("x10*  ")
    spinbox_Dist.setSingleStep(1)
    spinbox_Dist.valueChanged.connect(value_changed_DistObs)
    spinbox_Dist.show()
