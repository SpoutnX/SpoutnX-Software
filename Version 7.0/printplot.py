...
windowOG_r = QWidget()
windowOG_r.setWindowTitle("Résultats")
windowOG_r.setFixedSize(QSize(600, 450))
windowOG_r.setWindowIcon(QIcon('/usr/lib/spoutnx/icone2.ico'))

windowOG_Pt = QWidget()
windowOG_Pt.setWindowTitle("Résultats")
windowOG_Pt.setFixedSize(QSize(600, 450))
windowOG_Pt.setWindowIcon(QIcon('/usr/lib/spoutnx/icone2.ico'))
...
def ModelOG_r():
    try:
        mes_nombres = open(path4).read()
        L = mes_nombres.split('\n')
        FigOG_r = FigureCanvasQTAgg()
        fig, FigOG_r.ax = plt.subplots(figsize=(5, 5), dpi=100)
        FigOG_r.setParent(windowOG_r)

        Data_r = pd.read_excel(path5, usecols="B")
        ax_y = list(Data_r["r"])

        Data_T = pd.read_excel(path5, usecols="G")
        ax_x = list(Data_T["Time"])

        FigOG_r.ax.plot(ax_x, ax_y, color='red', lw=3)
        FigOG_r.ax.set(title=L[210])
        FigOG_r.ax.set_ylabel(L[213])
        FigOG_r.ax.set_xlabel(L[212])
        FigOG_r.ax.set_yscale("log")
        FigOG_r.ax.grid()
        fig.show()
        FigOG_r.show()
        fig.savefig(path6)
...
def ModelOG_Pt():
    try:
        ...
        FigOG_r = FigureCanvasQTAgg()
        fig, FigOG_r.ax = plt.subplots(figsize=(5, 5), dpi=100)
        FigOG_r.setParent(windowOG_Pt)

        Data_r = pd.read_excel(path5, usecols="C")
        ax_y = list(Data_r["Pt"])
        ...
        FigOG_r.ax.plot(ax_x, ax_y, color='red', lw=3)
        FigOG_r.ax.set(title=L[215])
        FigOG_r.ax.set_ylabel(L[214])
        FigOG_r.ax.set_xlabel(L[212])
        FigOG_r.ax.set_yscale("log")
        FigOG_r.ax.grid()
        fig.show()
...
def ModelOG_V(v1, v2):
    try:
        ...
        if v1 == 1:
            Data_r = pd.read_excel(path5, usecols="D")
            ax_y = list(Data_r["v1"])
            FigOG_r.ax.plot(ax_x, ax_y, color='red', lw=3)

        if v2 == 1:
            Data_r = pd.read_excel(path5, usecols="E")
            ax_y2 = list(Data_r["v2"])
            FigOG_r.ax.plot(ax_x, ax_y2, color='green', lw=3)

        FigOG_r.ax.set(title=L[216])
        FigOG_r.ax.set_ylabel(L[211])
        FigOG_r.ax.set_xlabel(L[212])
        FigOG_r.ax.set_yscale("log")
        FigOG_r.ax.grid()
        fig.show()
